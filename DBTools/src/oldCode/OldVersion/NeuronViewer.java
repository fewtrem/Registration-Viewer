...
public class NeuronViewer {
	...
	public NeuronViewer(String sourceFolder,ArrayList<HashMap<String,String>> fileSys,HashMap<String,String> replacementValues,ImagePlus templateImg,boolean editor,String origPre){
		...
		curNeuron = null;
		// Get lists:
		this.origFileList = RegViewer_.findFileNamesOfType(fileSys,RegViewer_.TYPE_ORIG);
		this.neuronFileList = RegViewer_.findFileNamesOfType(fileSys,RegViewer_.TYPE_NEURON);
	...
	public ArrayList<String> getListOfTabs(){
		...
		for (HashMap<String,String> fileInfo : fileSys){
			...
			if (fileInfo.get(RegViewer_.TYPE).equals(RegViewer_.TYPE_ORIG)||
					fileInfo.get(RegViewer_.TYPE).equals(RegViewer_.TYPE_NEURON)){
				String thisTab;
				...
				if (fileInfo.get(RegViewer_.TYPE).equals(RegViewer_.TYPE_ORIG)){
					origTabList.put(fileInfo.get(RegViewer_.NAME), thisTab);
				}
				else if (fileInfo.get(RegViewer_.TYPE).equals(RegViewer_.TYPE_NEURON)){
					neuronTabList.put(fileInfo.get(RegViewer_.NAME), thisTab) ;
				}
		...
		for (HashMap<String,String> fileInfo : fileSys){
			if (fileInfo.get(RegViewer_.TYPE).equals(TYPE_TAB)){
				String thisTabKey = fileInfo.get(RegViewer_.NAME);
				int thisTabNum;
		...
		for (String thisTab : tabs){
			...
			for (String thisFile : origFileList.keySet()){
				if (origTabList.get(thisFile).equals(thisTab)){
					JLabel toAdd = generateImageLabel(RegViewer_.TYPE_ORIG,thisFile);
			...
			for (String thisFile : neuronFileList.keySet()){
				if (neuronTabList.get(thisFile).equals(thisTab)){
					JLabel toAdd = generateImageLabel(RegViewer_.TYPE_NEURON,thisFile);
	...
	private JLabel generateImageLabel(String type,String name){
		...
		for (HashMap<String,String> fileInfo : fileSys){
			if (fileInfo.get(RegViewer_.TYPE).equals(type) && fileInfo.get(RegViewer_.NAME).equals(name)){
				retLabel = new JLabel();
				doneIt = true;
				retLabel.setText("");
				...
				retLabel.putClientProperty(RegViewer_.NAME,name);
				retLabel.putClientProperty(RegViewer_.TYPE,type);
				retLabel.addMouseListener(new MouseAdapter(){
				    @Override
				    public void mouseClicked(MouseEvent e){
				    	// Lazy as can't be bothered to create extended Jlabel class....
				    	JLabel source = (JLabel) e.getSource(); 
				    	String name = (String) source.getClientProperty(RegViewer_.NAME);
				    	String type = (String) source.getClientProperty(RegViewer_.TYPE);
	...
	public void openDoubleClick(String type,String name){
		if (curNeuron != null){
			...
			if (type.equals(RegViewer_.TYPE_NEURON)){
				thisIP = curNeuron.getImage(name);
			}
			else if (type.equals(RegViewer_.TYPE_ORIG))
				thisIP = origImages.get(curNeuron.getXMLData(RegViewer_.FOLDER)).get(name);
			if (thisIP != null){
	...
	private String getTabName(String tabName){
		...
		for (HashMap<String,String> fileInfo : fileSys){
			if (fileInfo.get(RegViewer_.TYPE).equals(TYPE_TAB)&&fileInfo.get(RegViewer_.NAME).equals(tabName)){
	...
	private void generateText(JPanel pane){
		...
		for (HashMap<String,String> fileInfo : fileSys){
			if (fileInfo.get(RegViewer_.TYPE).equals(RegViewer_.TYPE_TEXT)||fileInfo.get(RegViewer_.TYPE).equals(RegViewer_.TYPE_TEXTFIELD)){
		...
		for (HashMap<String,String> fileInfo : fileInfoSorter){
			if (fileInfo.get(RegViewer_.TYPE).equals(RegViewer_.TYPE_TEXT)){
				generateTextLabel(pane,fileInfo);
			}
			if (fileInfo.get(RegViewer_.TYPE).equals(RegViewer_.TYPE_TEXTFIELD)){
	...
	private void generateTextLabel(JPanel pane,HashMap<String,String> fileInfo){
		...
		this.textLabelsText.put(fileInfo.get(RegViewer_.NAME),textToSave);
		this.textLabels.put(fileInfo.get(RegViewer_.NAME),retLabel);
	...
	private void generateTextField(JPanel pane,HashMap<String,String> fileInfo){
		...
		this.textFields.put(fileInfo.get(RegViewer_.NAME),retTF);
		if (editor==false){
	...
	public void update(Neuron newNeuron){
		...
		for (String thisKey : textLabels.keySet()){
			textLabels.get(thisKey).setText(RegViewer_.doReplacement(textLabelsText.get(thisKey), replacementValues, newNeuron));
		}
	...
	public void getOrigImages(Neuron inNeuron){
		...
		String folder = inNeuron.getXMLData(RegViewer_.FOLDER);
		...
		if (continueThis==true){
			if (origImages.containsKey(thisFolder)==false){
				...
				for (String key :origFileList.keySet()){
					RegViewer_.openImage(key,origFileList.get(key),inNeuron,toSave);
				}
	...
	public void openOrigStack(Neuron neuronIN){
		...
		ImagePlus neuronOrig = RegViewer_.loadImage(thisFilePath);
		neuronOrig.show();
