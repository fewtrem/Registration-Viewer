...
public class RegStackViewer {
	...
	public static ImagePlus getRegStack(Neuron neuronToView,String sourceFolder,ImagePlus templateStack,HashMap<String,String> replacementValues){
		...
		if (templateStack!=null && neuronToView!=null){
			String newFilename = RegViewer_.doReplacement(neuronToView.getXMLData(NRRDFILE), replacementValues, neuronToView);
			File thisFilePath = new File(sourceFolder,neuronToView.getXMLData(RegViewer_.FOLDER));
			thisFilePath = new File(thisFilePath,newFilename);
			ImagePlus neuronNRRD = RegViewer_.loadImage(thisFilePath);
			if (neuronNRRD != null){
