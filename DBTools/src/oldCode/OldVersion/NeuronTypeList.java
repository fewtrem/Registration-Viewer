...
public class NeuronTypeList {
	...
	private RegViewer_ theOverallProg;
	// GUI stuff:
	private JFrame dialog;
	private JList chooseAValue;
	...
	public JPanel generateButtons(){
		...
		if (editor==true){
			// changeDBName button:
			Button changeDBNameB = new Button("Change neuron class");
			...
			buttonList.add(changeDBNameB);
			// set best in class button:
			Button setBestInClassB = new Button("Set best in class");
			...
			buttonList.add(setBestInClassB);
			// set best in class button:
			Button setNotBestInClassB = new Button("Set NOT best in class");
			...
			buttonList.add(setNotBestInClassB);
		}
		// open registered stack button:
		Button openRegisteredStackB = new Button("Open registered stack");
		...
		buttonList.add(openRegisteredStackB);
		// open original stack button:
		Button openOriginalStack = new Button("Open original stack");
		...
		buttonList.add(openOriginalStack);
	...
	public void changeTheDBNameDialog(){
		if (exampleList.isSelectionEmpty()==true){
			...
			if (theOverallProg!=null){
				theOverallProg.setEnabled(false);
			}
			Neuron neuronIN = exampleListCorrespondingNeurons.get(exampleList.getSelectedIndex());
			...
			// Jlist:
			chooseAValue = new JList(invalues.toArray(new String[invalues.size()]));
	...
	public void updateViewer(){
		if (theViewer == null){
			...
			if (exampleList.isSelectionEmpty()==true){
				...
				neuronIN = exampleListCorrespondingNeurons.get(exampleList.getSelectedIndex());
				// grey in the buttons
				for (Button thisButton : buttonList){
	...
	public void updateExampleList(boolean setSelected){
		...
		if (nonSelected==false){
			oldSelectedNeuron = exampleListCorrespondingNeurons.get(exampleList.getSelectedIndex());
		}
		else{
		...
		if (ourJList.isSelectionEmpty()==false){
			...
			for (Neuron thisNeuron : neuronListToSort){
				toList.add(thisNeuron.toString());
		...
		if (setSelected==true){
			exampleList.setListData(toList.toArray(new String[toList.size()]));
			if (newSelectedIndex != -1){
	...
	// Tested, Reviewed 23/07/2016
	public void setRegViewer(RegViewer_ inProg){
